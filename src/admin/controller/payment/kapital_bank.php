...
class KapitalBank extends \Opencart\System\Engine\Controller {
    ...
    public function index(): void {
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('extension/kapital_bank/payment/kapital_bank', 'user_token=' . $this->session->data['user_token'], true)
        ];

        $data['action'] = $this->url->link('extension/kapital_bank/payment/kapital_bank.save', 'user_token=' . $this->session->data['user_token'], true);
        $data['action_files'] = $this->url->link('extension/kapital_bank/payment/kapital_bank.save_files', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/kapital_bank/payment/kapital_bank', $data));
    }
    ...
    public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/kapital_bank/payment/kapital_bank')) {
			$json['error'] = $this->language->get('error_permission');
		}
        
        if (!$this->validate()) {
            $json['error'] = $this->language->get('taksit_months_absent');
        } else if (!$json && $this->request->server['REQUEST_METHOD'] == 'POST') {
			$this->load->model('setting/setting');
    
            if (isset($this->request->post['payment_kapital_bank_certificate'])) {
                $this->request->post['payment_kapital_bank_certificate'] = $this->request->post['payment_kapital_bank_certificate'];
            } else {
                $this->request->post['payment_kapital_bank_certificate'] = $this->config->get('payment_kapital_bank_certificate');
            }
    
            if (isset($this->request->post['payment_kapital_bank_certificate_key'])) {
                $this->request->post['payment_kapital_bank_certificate_key'] = $this->request->post['payment_kapital_bank_certificate_key'];
            } else {
                $this->request->post['payment_kapital_bank_certificate_key'] = $this->config->get('payment_kapital_bank_certificate_key');
            }
	...
	public function save_files(): void {
        $this->load->language('extension/kapital_bank/payment/kapital_bank');
		...
		if (!$json && ($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if (isset($this->request->post['payment_kapital_bank_status'])) {
                $this->request->post['payment_kapital_bank_status'] = (int)$this->request->post['payment_kapital_bank_status'];
            } else {
                $this->request->post['payment_kapital_bank_status'] = $this->config->get('payment_kapital_bank_status');
            }
            
            if (isset($this->request->post['payment_kapital_bank_test'])) {
                $this->request->post['payment_kapital_bank_test'] = $this->request->post['payment_kapital_bank_test'];
            } else {
                $this->request->post['payment_kapital_bank_test'] = $this->config->get('payment_kapital_bank_test');
            }
    
            if (isset($this->request->post['payment_kapital_bank_taxes'])) {
                $this->request->post['payment_kapital_bank_taxes'] = $this->request->post['payment_kapital_bank_taxes'];
            } else {
                $this->request->post['payment_kapital_bank_taxes'] = $this->config->get('payment_kapital_bank_taxes');
            }
    
            if (isset($this->request->post['payment_kapital_bank_merchant_id'])) {
                $this->request->post['payment_kapital_bank_merchant_id'] = $this->request->post['payment_kapital_bank_merchant_id'];
            } else {
                $this->request->post['payment_kapital_bank_merchant_id'] = $this->config->get('payment_kapital_bank_merchant_id');
            }
    
            if (isset($this->request->post['payment_kapital_bank_taksit_check'])) {
                $this->request->post['payment_kapital_bank_taksit_check'] = $this->request->post['payment_kapital_bank_taksit_check'];
            } else {
                $this->request->post['payment_kapital_bank_taksit_check'] = $this->config->get('payment_kapital_bank_taksit_check');
            }
    ...
    private function save_file($key) {
        ...
        if (is_uploaded_file($file['tmp_name'])) {
            if (!file_exists(DIR_EXTENSION . 'kapital_bank/catalog/model/kapital_bank/certificates')) {
                mkdir(DIR_EXTENSION . 'kapital_bank/catalog/model/kapital_bank/certificates', 755, true);
            }
            if ($current_file) {
                @unlink(DIR_EXTENSION . 'kapital_bank/catalog/model/kapital_bank/certificates/' . $current_file);
            }
            $name = $file['name'];
            if (move_uploaded_file($file['tmp_name'], DIR_EXTENSION . 'kapital_bank/catalog/model/kapital_bank/certificates/' . $name)) {
    ...
    public function install() {
        ...
        $this->model_extension_kapital_bank_payment_kapital_bank->install();
    }

    public function uninstall() {
        $this->load->model('extension/kapital_bank/payment/kapital_bank');
        $this->model_extension_kapital_bank_payment_kapital_bank->uninstall();
    }

    public function order() {
        $this->load->model('extension/kapital_bank/payment/kapital_bank');
        require_once DIR_EXTENSION . 'kapital_bank/admin/model/kapital_bank/PaymentAdapter.php';
        require_once DIR_EXTENSION . 'kapital_bank/admin/model/kapital_bank/KapitalBankPaymentAdapter.php';
        $order = $this->model_extension_kapital_bank_payment_kapital_bank->getOrder($this->request->get['order_id']);
        ...
        $data = [];
        $refunds = $this->model_extension_kapital_bank_payment_kapital_bank->getRefunds($this->request->get['order_id']);
        try {
        ...
        return $this->load->view('extension/kapital_bank/payment/kapital_bank_transaction', $data);
    }
        ...
        try {
            if ($this->request->post['refund_amount'] > 0 && ($order_total - $this->request->post['refund_amount']) >= 0) {
                if ($adapter->refundOrder(strip_tags($this->request->post['refund_desc']), $this->request->post['refund_amount'] * 100)) {
                    $this->model_extension_kapital_bank_payment_kapital_bank->addRefund($this->request->get['order_id'], $this->request->post['refund_amount'], $this->request->post['currency'], strip_tags($this->request->post['refund_desc']));
