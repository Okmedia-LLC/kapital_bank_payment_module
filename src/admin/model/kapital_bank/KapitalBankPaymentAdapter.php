...
class KapitalBankPaymentAdapter extends PaymentAdapter {
    ...
    public function __construct($plugin_id, $merchant, $certificate, $key, $test_mode = true) {
        ...
        $dir = defined('DIR_EXTENSION') ? DIR_EXTENSION : DIR_APPLICATION;

        $this->cert = $test_mode ? $dir . 'kapital_bank/catalog/model/kapital_bank/certificates/test/test.crt' : $dir . 'kapital_bank/catalog/model/kapital_bank/certificates/' . $certificate;
        $this->key = $test_mode ? $dir . 'kapital_bank/catalog/model/kapital_bank/certificates/test/test.key' : $dir . '.kapital_bank/catalog/model/kapital_bank/certificates/' . $key;
        $this->merchant = $test_mode ? 'E1000010' : $merchant;
        $this->mode = $test_mode ? 'test' : 'prod';
        if($this->mode=='test'){
            $this->merchant_handler = "https://tstpg.kapitalbank.az:5443/Exec";
            $this->client_handler = "https://tstpg.kapitalbank.az/index.jsp";
        }else{
     ...
     */
    public function setKapitalModel($kapital_bank_model): void {
     ...
     */
    public function setModelCheckoutOrder($model_checkout_order): void {
...
<TKKPG>
  <Request>
   ...
   <SessionID>' . $this->pay_details['ecommerce_sessionid'] . '</SessionID>
        ...
        if (curl_error($ch)) {
            $this->model_checkout_order->addHistory($this->order, 10, 'Error: ' . curl_error($ch));
    ...
    public function create() {
        ...
        if ($xmlObj->Response->Order->SessionID) {
            ...
            $this->model_checkout_order->addHistory($this->order, 2, 'Processing order');
        } else {
            $this->model_checkout_order->addHistory($this->order, 10, 'Error processing: ' . htmlspecialchars($xmlObj));
                ...
                <TKKPG>
                    <Request>
                        ...
                        <Order>
                            ...
                            <ApproveURL><![CDATA[' . HTTP_SERVER . 'index.php?route=extension/kapital_bank/payment/kapital_bank.callback&action=approve&id=' . $this->order . ']]></ApproveURL>
                            <CancelURL><![CDATA[' . HTTP_SERVER . 'index.php?route=extension/kapital_bank/payment/kapital_bank.callback&action=cancel&id=' . $this->order . ']]></CancelURL>
                            <DeclineURL><![CDATA[' . HTTP_SERVER . 'index.php?route=extension/kapital_bank/payment/kapital_bank.callback&action=decline&id=' . $this->order . ']]></DeclineURL>
                        </Order>
                    </Request>
                </TKKPG>';
    ...
    private function curl($xml_string) {
        ...
        if (!file_exists($this->cert)) {
            throw new \Exception('Certificate file not found: ' . $this->cert);
        }
        if (!file_exists($this->key)) {
            throw new \Exception('Key file not found: ' . $this->key);
        }
        ...
        if (curl_errno($ch)) {
            ...
            if ($this->model_checkout_order) {
                $this->model_checkout_order->addHistory($this->order, 10, 'Error: ' . $error_msg);
            }
